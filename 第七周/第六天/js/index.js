var searchRender = (function () {
    ...
    return {
        init: function () {
            //->给文本框的获取焦点和输入内容事件绑定方法:当操作的时候,容如果文本框中有内,我们向百度的服务器发送一个JSONP请求,把用户输入的内容传递给百度,百度返回对应的推荐结果;如果有推荐的结果,我们展示UL,没有的话我们隐藏UL;
            $searchInp.on('focus keyup', inpEven);

            //->点击不同的位置实现不同的操作(事件委托,委托给BODY来处理):点击的是文本框什么都不做,如果点击的是UL下的LI，我们不仅要隐藏UL，还要把LI中的内容复制到文本框中，如果点击的是其他区域只需要隐藏UL即可
